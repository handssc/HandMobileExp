...
{
    NSMutableArray *imageArray;             // Image Array
    EXPScrollview *_scrollview;             // 滚动视图
...
@property (nonatomic ,strong) NSString *todaySum;           // today

@property (nonatomic ,strong) NSString *weekSum;            // this week

@property (nonatomic ,strong) NSString *monthSum;           // this month
...
- (void) handleTimer: (NSTimer *) timer
{
    if (TimeNum % 5 == 0 && TimeNum != 0) {
        NSInteger page = _scrollview.pagecontrol.currentPage; // 获取当前的page
        page++;
        page = page > 5 ? 0 : page ;
...
-(void)modelDidFinishLoad:(FMDataBaseModel *)model{
    ...
    double weekSumTotal = 0;
    double monthSumTotal = 0;
    double todaySumTotal = 0;
    ...
    for (  NSDictionary * record in  model.result){
        ...
        if ([weekDate compare:[record objectForKey:@"expense_date"]] <= 0 && [lastDayOfWeek compare:[record objectForKey:@"expense_date"]] >=0) {
            ...
            ;
            weekSumTotal  += [ [record objectForKey:@"total_amount"] doubleValue ];
        ...
        if ([monthDate compare:[record objectForKey:@"expense_date"]] <= 0 && [lastDayOfMonth compare:[record objectForKey:@"expense_date"]] >=0) {
            ...
            //*[[record objectForKey:@"expense_number"] integerValue];
            monthSumTotal += [ [record objectForKey:@"total_amount"] doubleValue ];
    ...
    self.monthSum = [NSString stringWithFormat:@"¥%2.2f",monthSumTotal];
    self.todaySum = [NSString stringWithFormat:@"¥%2.2f",todaySumTotal];
    
    
}

/******************************************************
 **************** del by wuxiaocheng @2015/2/4 for 改为总额
